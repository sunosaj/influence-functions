...
model = SmoothHinge(
    ...
    train_dir='output',
    log_dir='log',
    model_name='fashion_smooth_hinge_17_t-%s' % temp)
...
for counter, temp in enumerate(temps):
    ...
    if temp == 0:
        actual_loss_diffs[counter, :], predicted_loss_diffs[counter, :], indices_to_remove[counter,
                                                                         :] = experiments.test_retraining(
            model,
            test_idx,
            iter_to_load=0,
            force_refresh=False,
