...
# minority_group = 'Amer-Indian-Eskimo'
# minority_group = 'Asian-Pac-Islander'
...
preds = preds[0]
rocauc_score = roc_auc_score(tf_model.data_sets.test.labels, preds[:, 1])
print('rocauc_score', rocauc_score)
# Model Performance

female_indices = np.argwhere(X_train[:, 61] == 1)
male_indices = np.argwhere(X_train[:, 62] == 1)
amerindianeskimo_indices = np.argwhere(X_train[:, 56] == 1)
asianpacislander_indices = np.argwhere(X_train[:, 57] == 1)
black_indices = np.argwhere(X_train[:, 58] == 1)
other_indices = np.argwhere(X_train[:, 59] == 1)
white_indices = np.argwhere(X_train[:, 60] == 1)

print(female_indices.shape)
print(male_indices.shape)
print(amerindianeskimo_indices.shape)
print(asianpacislander_indices.shape)
print(black_indices.shape)
print(other_indices.shape)
print(white_indices.shape)
...
if minority_group != '':
    ...
    elif minority_group == 'Amer-Indian-Eskimo':
        minority_group_indices = amerindianeskimo_indices
    elif minority_group == 'Asian-Pac-Islander':
        minority_group_indices = asianpacislander_indices
    elif minority_group == 'Black':
        minority_group_indices = black_indices
...
for flips_idx in range(num_flip_vals):
    for random_seed_idx in range(num_random_seeds):
        ...
        for checks_idx in range(num_check_vals):
            ...
            female_count = np.argwhere(samples_to_check[:, 61] == 1).shape[0]
            male_count = np.argwhere(samples_to_check[:, 62] == 1).shape[0]
            amerindianeskimo_count = np.argwhere(samples_to_check[:, 56] == 1).shape[0]
            asianpacislander_count = np.argwhere(samples_to_check[:, 57] == 1).shape[0]
            black_count = np.argwhere(samples_to_check[:, 58] == 1).shape[0]
            other_count = np.argwhere(samples_to_check[:, 59] == 1).shape[0]
            white_count = np.argwhere(samples_to_check[:, 60] == 1).shape[0]
            ...
            proportion_counts_influence[flips_idx, random_seed_idx, checks_idx, 2] = amerindianeskimo_count
            proportion_counts_influence[flips_idx, random_seed_idx, checks_idx, 3] = asianpacislander_count
            proportion_counts_influence[flips_idx, random_seed_idx, checks_idx, 4] = black_count
            proportion_counts_influence[flips_idx, random_seed_idx, checks_idx, 5] = other_count
            proportion_counts_influence[flips_idx, random_seed_idx, checks_idx, 6] = white_count
            ...
            print(female_count)
            print(male_count)
            print(amerindianeskimo_count)
            print(asianpacislander_count)
            print(black_count)
            print(other_count)
            print(white_count)
            ...
            proportion_counts_loss[flips_idx, random_seed_idx, checks_idx, 2] = amerindianeskimo_count
            proportion_counts_loss[flips_idx, random_seed_idx, checks_idx, 3] = asianpacislander_count
            proportion_counts_loss[flips_idx, random_seed_idx, checks_idx, 4] = black_count
            proportion_counts_loss[flips_idx, random_seed_idx, checks_idx, 5] = other_count
            proportion_counts_loss[flips_idx, random_seed_idx, checks_idx, 6] = white_count
            ...
            proportion_counts_random[flips_idx, random_seed_idx, checks_idx, 2] = amerindianeskimo_count
            proportion_counts_random[flips_idx, random_seed_idx, checks_idx, 3] = asianpacislander_count
            proportion_counts_random[flips_idx, random_seed_idx, checks_idx, 4] = black_count
            proportion_counts_random[flips_idx, random_seed_idx, checks_idx, 5] = other_count
            proportion_counts_random[flips_idx, random_seed_idx, checks_idx, 6] = white_count
            # look at proportion of protected groups in most influential samples #######################################
...
if minority_group != '':
    if minority_group == 'Female':
        save_file_name = 'output/adult_results_minority_female'
    elif minority_group == 'Amer-Indian-Eskimo':
        save_file_name = 'output/adult_results_minority_amerindianeskimo'
    elif minority_group == 'Asian-Pac-Islander':
        save_file_name = 'output/adult_results_minority_asianpacislander'
    elif minority_group == 'Black':
        save_file_name = 'output/adult_results_minority_black'
    elif minority_group == 'Other':
        save_file_name = 'output/adult_results_minority_other'
elif minority_group == '':
    save_file_name = 'output/adult_results'
