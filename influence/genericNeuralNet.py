...
def variable(name, shape, initializer):
    ...
    # Variables are manipulated via the tf.Variable class. A tf.Variable represents a tensor whose value can be changed
    # by running ops on it. Unlike tf.Tensor objects, a tf.Variable exists outside the context of a single session.run
    # call. Internally, a tf.Variable stores a persistent tensor. Specific ops allow you to read and modify the values
    # of this tensor. These modifications are visible across multiple tf.Sessions, so multiple workers can see the same
    # values for a tf.Variable.
    #
...
class GenericNeuralNet(object):
    ...
    def __init__(self, **kwargs):
        ...
        # I add
        self.all_validation_feed_dict = self.fill_feed_dict_with_all_ex(self.data_sets.validation)
        # I add
    ...
    def get_vec_to_list_fn(self):
        ...
        # print('len(params_val)', len(params_val))
    ...
    def update_learning_rate(self, step):
        # print("self.num_train_examples", self.num_train_examples)
    ...
    def get_fmin_loss_fn(self, v):
        ...
        def get_fmin_loss(x):
            ...
            # is_pos = np.all(np.linalg.eigvals(hessian_vector_val))
            # print('is_pos > 0', is_pos > 0)
            # print('is_pos >= 0', is_pos >= 0)
    ...
    def get_train_grad_loss_no_reg_val(self, test_indices, batch_size=100, loss_type='normal_loss'):
        ...
        if test_indices is not None:
            ...
            for i in range(num_iter):
                ...
                test_feed_dict = self.fill_feed_dict_with_some_ex(self.data_sets.train, test_indices[start:end])
            ...
            test_grad_loss_no_reg_val = self.minibatch_mean_eval([op], self.data_sets.train)[0]
    ...
    #     # print('\n')
    #     # print(self.sess.run(self.params)[1].shape)
    #     # print('\n')
    #
    ...
    #
    #     # print('\n')
    #     # print('X_train.shape', X_train.shape) # X_train.shape = (4137, 4769)
    #     # print('theta.T.shape', theta.T.shape) # theta.T.shape = (4769,)
    #     # print('\n')
